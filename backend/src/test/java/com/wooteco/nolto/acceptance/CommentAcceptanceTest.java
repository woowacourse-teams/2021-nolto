...
public class CommentAcceptanceTest extends AcceptanceTest {
    ...
    public void updateCommentWithAuthor() {
        ...
        // when
        ExtractableResponse<Response> 댓글_수정_응답 = 댓글을_수정한다(댓글_수정_요청, 현재_로그인된_댓글_작성자의_토큰.getAccessToken(), 등록된_댓글.getId());
        ExtractableResponse<Response> 댓글내용_빈값으로_수정_응답 = 댓글을_수정한다(댓글내용_빈값으로_수정_요청, 현재_로그인된_댓글_작성자의_토큰.getAccessToken(), 등록된_댓글.getId());
    ...
    public void updateReplyWithAuthor() {
        ...
        // when
        ExtractableResponse<Response> 대댓글_수정_응답 = 댓글을_수정한다(대댓글_수정_요청, 현재_로그인된_댓글_작성자의_토큰.getAccessToken(), 등록된_대댓글.getId());
    ...
    public void updateReplyWithHelperIsTrue() {
        ...
        // when
        ExtractableResponse<Response> 도와드릴게요로_대댓글_수정_응답 = 댓글을_수정한다(대댓글_도와드릴게요로_수정_요청, 현재_로그인된_댓글_작성자의_토큰.getAccessToken(), 등록된_대댓글.getId());
    ...
    public void deleteCommentWithAuthor() {
        ...
        // when
        ExtractableResponse<Response> 댓글_삭제_응답 = 댓글을_삭제한다(현재_로그인된_댓글_작성자의_토큰.getAccessToken(), 등록된_댓글.getId());
    ...
    public void deleteReplyWithAuthor() {
        ...
        // when
        ExtractableResponse<Response> 대댓글_삭제_응답 = 댓글을_삭제한다(현재_로그인된_댓글_작성자의_토큰.getAccessToken(), 등록된_대댓글.getId());
    ...
    public void addCommentLikeWithLoginUser() {
        ...
        // when
        ExtractableResponse<Response> 댓글_좋아요_추가_응답 = 댓글에_좋아요를_추가한다(현재_로그인된_댓글_작성자의_토큰.getAccessToken(), 등록된_댓글.getId());
    ...
    public void addCommentLikeWithAlreadyLiked() {
        ...
        // when
        ExtractableResponse<Response> 이미_좋아요_누른_댓글에_댓글_좋아요_추가_응답 = 댓글에_좋아요를_추가한다(현재_로그인된_댓글_작성자의_토큰.getAccessToken(), 등록된_댓글.getId());
    ...
    public void deleteCommentLikeWithLoginUser() {
        ...
        // when
        ExtractableResponse<Response> 댓글에_좋아요_취소_응답 = 댓글에_좋아요를_취소한다(현재_로그인된_댓글_작성자의_토큰.getAccessToken(), 등록된_댓글.getId());
    ...
    public void createReplyWithoutLogin() {
        ...
        // when
        ExtractableResponse<Response> 로그인_하지않고_대댓글_작성_응답 = 로그인_하지않고_대댓글을_작성한다(대댓글_작성_요청1, 등록된_댓글.getId());
    ...
    public void createReplyWithLoginUser() {
        ...
        // when
        ExtractableResponse<Response> 대댓글_작성_응답 = 대댓글을_작성한다(대댓글_작성요청, 현재_로그인된_댓글_작성자의_토큰.getAccessToken(), 등록된_댓글.getId());
        ExtractableResponse<Response> 대댓글_내용이_빈_값인_작성 = 대댓글을_작성한다(대댓글_내용이_빈_값인_작성요청, 현재_로그인된_댓글_작성자의_토큰.getAccessToken(), 등록된_댓글.getId());
    ...
    public void findAllRepliesByCommentIdWithLoginUser() {
        ...
        // when
        ExtractableResponse<Response> 대댓글_목록_조회_응답 = 로그인_하고_대댓글_목록_조회한다(등록된_댓글.getId(), 현재_로그인된_댓글_작성자의_토큰.getAccessToken());
    ...
    public ExtractableResponse<Response> 로그인_하고_대댓글_목록_조회한다(Long 등록한_댓글_ID, String accessToken) {
        return given().log().all()
                .auth().oauth2(accessToken)
                .contentType(MediaType.APPLICATION_JSON_VALUE)
                .pathParam("feedId", 업로드한_피드의_ID)
    ...
    public void 댓글에_좋아요_추가_되어있음(Long 등록된_댓글_ID) {
        댓글에_좋아요를_추가한다(로그인된_댓글_작성자의_토큰.getAccessToken(), 등록된_댓글_ID);
    }

    public void 대댓글에_좋아요_추가_되어있음(Long 등록된_댓글_ID) {
        댓글에_좋아요를_추가한다(로그인된_댓글_작성자의_토큰.getAccessToken(), 등록된_댓글_ID);
    }
    ...
    public CommentResponse 대댓글_등록되어_있음(CommentRequest request, Long commentId) {
        return 대댓글을_작성한다(request, 로그인된_댓글_작성자의_토큰.getAccessToken(), commentId).as(CommentResponse.class);
    }
    ...
    public Long 피드_업로드되어있음(FeedRequest request) {
        return Long.valueOf(피드를_작성한다(request, 로그인된_댓글_작성자의_토큰.getAccessToken()).header("Location").replace("/feeds/", ""));
    }
