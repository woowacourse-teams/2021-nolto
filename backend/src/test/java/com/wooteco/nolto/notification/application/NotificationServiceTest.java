...
class NotificationServiceTest {
    ...
    private void 알림_타입에_따라_동등성을_비교한다(List<Notification> notifications) {
        for (Notification notification : notifications) {
            if (notification.getNotificationType().equals(NotificationType.LIKE)) {
                피드에_좋아요를_받은_경우_알림이_올바르게_저장된다(notification);
                continue;
            }

            if (notification.getNotificationType().equals(NotificationType.COMMENT)) {
                피드에_댓글이_달린_경우_알림이_올바르게_저장된다(notification);
                continue;
            }

            피드에_도움을_제안하는_댓글이_달린_경우_알림이_올바르게_저장된다(notification);
        }
    }
